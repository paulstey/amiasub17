...
\begin{algorithm}
\caption{AdaBoost.M1}\label{adaboost}
\begin{algorithmic}[1]
	\Procedure{adaboost}{}
		...
		\For{$t = 1...T$}
			\State Fit classifier $h_t(x)$ to training data using $w_i$
			\State Compute error: 
				\State $$\epsilon_t = \frac{\sum_{i=1}^{N} w_i I(y_i \ne h_t(x_i))}{\sum_{i = 1}^{N} w_i}$$
			\State Compute $\alpha_t$:
				\State $$\alpha_t = \frac{1}{2} \text{log} \left(\frac{1 - \epsilon_t}{\epsilon_t} \right)$$
			\State Set weights for next round:
				\State $$w_i \gets w_i \cdot \text{exp}$$
		\EndFor
	\EndProcedure
\end{algorithmic}
\end{algorithm}
